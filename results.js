...
function generateAnswersReview(quizData, userAnswers) {
    ...
    quizData.forEach((question, index) => {
        ...
        answerDiv.className = 'glass-effect rounded-2xl p-6 flex items-start gap-4';
        
        // Icon
        const iconDiv = document.createElement('div');
        iconDiv.className = `flex-shrink-0 ${isCorrect ? 'text-green-400 bg-green-400/20' : 'text-red-400 bg-red-400/20'} rounded-xl p-3`;
        ...
        questionP.className = 'font-semibold text-white text-lg';
        questionP.textContent = `Challenge ${index + 1}: ${question.question}`;
        
        const userAnswerP = document.createElement('p');
        userAnswerP.className = 'text-sm text-white/70 mt-2';
        ...
        if (userAnswer !== null && userAnswer !== undefined) {
            ...
            userAnswerP.innerHTML = `Your answer: <span class="font-semibold text-white/50">Skipped</span>`;
        }
        
        const correctAnswerP = document.createElement('p');
        correctAnswerP.className = 'text-sm text-white/70 mt-1';
        ...
        statusSpan.className = `text-sm font-bold px-3 py-1 rounded-lg ${isCorrect ? 'text-green-900 bg-green-400' : 'text-red-900 bg-red-400'}`;
