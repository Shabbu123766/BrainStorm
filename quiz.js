...
function loadQuestion() {
    ...
    question.options.forEach((option, index) => {
        ...
        label.className = 'flex items-center gap-4 glass-effect rounded-2xl p-6 cursor-pointer transition-all duration-300 hover:bg-white/20 hover:scale-[1.02] has-[:checked]:option-selected';
        
        const input = document.createElement('input');
        input.type = 'radio';
        input.name = 'quiz-option';
        input.value = index;
        input.className = 'h-6 w-6 shrink-0 appearance-none rounded-full border-2 border-white/40 bg-transparent checked:border-white checked:bg-white focus:outline-none focus:ring-2 focus:ring-white/50';
...
function updateTimer(isTimedOutDisplay) {
    if (isTimedOutDisplay) {
        ...
        timerSpan.className = 'font-mono font-bold text-lg timer-danger';
        return;
    }
    ...
    if (timeLeft <= 10) {
        ...
        timerSpan.className = 'font-mono font-bold text-lg timer-warning';
    } else {
        timerSpan.className = 'font-mono font-bold text-lg text-white';
    }
}
